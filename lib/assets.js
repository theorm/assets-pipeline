...
var LESS_LIBRARY_TAG = "<script src=\"%s\"></script>"

var DEFAULT_LESS_SCRIPT_URL = "//cdnjs.cloudflare.com/ajax/libs/less.js/1.7.0/less.min.js";
...
var defaultOptions = {
  ...
  deployment: {},
  lessScriptUrl : DEFAULT_LESS_SCRIPT_URL
}
...
Assets.prototype.lessToHtml = function(name) {
  if (this.options.production) {
    ...
    tags.push(fmt(LESS_LIBRARY_TAG, this.options.lessScriptUrl));
    return tags.join("\n");
  }
}

Assets.prototype.staticUrl = function(name) {
  if (this.options.production) {
    return this.options.productionPrefix + '/' + name;
  } else {
    return this.options.localPrefix + '/' + name;
  }
}
